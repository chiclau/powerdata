...
app.controller('companyUserManageController', ['$scope', '$http', 'uiGridConstants','$templateCache',
	function ($scope, $http, uiGridConstants, $templateCache) {
	
	// 用户类型集合
	$scope.userTypes = [
        { label: "全部类别",value: ''},
		{ label: "管理用户",value: 'QYYH_GLJS'},
		{ label: "填报用户",value: 'QYYH_TBJS'},
		{ label: "审核用户",value: 'QYYH_SHJS'}
    ];
	...
	$scope.baseGrid = {
		...
		gridOptions : {
			columnDefs: [                                                              //定义表格列内容       
				...
				{name : 'XTZH', displayName : '登录账号', width :'30%', cellClass : "grid-col-center"},
		...
		dataConfig : {																//配置数据接口
			url: Common.webRoot() + '/co/epsm/dataacquisition/companyuser/companyusercontroller/findsystemuser',
			data:{																	//配置数据过滤条件
				pageSize : Common.pageSize,
				pageNum:1,
				QYID : $scope.companyUser.QYID
			}
		},
		gridEvent : {                                                               //表格事件方法
			/*以下部分的函数名称是自定义的*/
			...
			userTypeShow : function (grid, row) {
				if (row.entity.YHJS) {
					return row.entity.YHJS.replace('QYYH_GLJS', '管理用户').replace('QYYH_TBJS', '填报用户').replace('QYYH_SHJS', '审核用户');
				}
				return "";
			},
			...
			delUser : function (grid, row) {
				...
				Common.dialog({
    				...
    				callback : function(index){
    					Common.send($scope, $http, {
    		        		method: 'POST',
    		        		url: Common.webRoot() + '/co/epsm/dataacquisition/companyuser/companyusercontroller/delcompanyuser',
    		        		showTips : true,
    		        		data: {
    		        			"YHID" : row.entity.YHID
		        			},
			...
			readUser : function (grid, row) {
				Common.dialog({
		            ...
		            height: '95%',
		            noScroll : 'true',
		            url: Common.webRoot() + '/co/epsm/dataacquisition/companyuser/companyusercontroller/companyuserdetail/read/' + row.entity.XTZH
		        });
			},
			editUser : function (grid, row) {
				Common.dialog({
		            ...
		            height: '95%',
		            noScroll : 'true',
		            url: Common.webRoot() + '/co/epsm/dataacquisition/companyuser/companyusercontroller/companyuserdetail/edit/' + row.entity.XTZH,
		            end: function(){
		            	Common.page.send($scope, $http, $scope.baseGrid);
		            }
		        });
	...
	$scope.add = function() {
		Common.dialog({
            ...
            height: '95%',
            noScroll : 'true',
            url: Common.webRoot() + '/co/epsm/dataacquisition/companyuser/companyusercontroller/companyuserdetail/add/no',
            end: function(){
            	Common.page.send($scope, $http, $scope.baseGrid);
            }
        });
